...
xhr.onload = function() {                       // When readystate changes
  ...
  if(xhr.status === 200) {                      // If server status was ok
    ...
    for (var i = 0; i < responseObject.orders.length; i++) { // Loop through object
        row[i] = new order(Number(responseObject.orders[i].OrderNum), Number(responseObject.orders[i].OrderNum), responseObject.orders[i].FirstName, responseObject.orders[i].LastName, responseObject.orders[i].Email, Number(responseObject.orders[i].CarNum), responseObject.orders[i].Make, responseObject.orders[i].Model, responseObject.orders[i].BodyStye, Number(responseObject.orders[i].Year), responseObject.orders[i].Color, responseObject.orders[i].Price, responseObject.orders[i].CarPicture);
    createOrder(row[i]);
    }
  }
    checkRadio();
...
function editOrder() {
    $(this).keypress(function(event){
        ...
        if(x == 13) {
            ...
            var colorEdit = editNode.getElementsByTagName("td")[7];
            if(this == colorEdit) {
    ...
    */
    if(_.where(carOrder,{"FirstName": searchContent.toUpperCase()}).length == 0
       ...
       && _.where(carOrder,{"CarPictur": searchContent}).length == 0
        ...
        //show up the text of no result
        $resultBox.show();
        $resultBox.text('Sorry, no result!');
    }
}
/* ---------- search function end ---------- */